        ...
        try {
            ...
            if (response.ok) {
                ...
                data.photos.photo.forEach((photo)=>{
                    let imgUrl;
                    if (size === "l") imgUrl = `https://live.staticflickr.com/${photo.server}/${photo.id}_${photo.secret}_b.jpg`;
                    else if (size === "m") imgUrl = `https://live.staticflickr.com/${photo.server}/${photo.id}_${photo.secret}_${size}.jpg`;
                    else if (size === "s") imgUrl = `https://live.staticflickr.com/${photo.server}/${photo.id}_${photo.secret}_${size}.jpg`;
                    displayImg(imgUrl);
                });
            } else throw new Error("Oops, n\xe5got gick fel!");
        } catch (error) {
            showError(error.message);
        }
    } else showError("Var god ange ett s\xf6kord");
});
...
function showError(message) {
    ...
    error.innerText = "Bilden som angivet kunde inte hittas, var god f\xf6rs\xf6k igen";
